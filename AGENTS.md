...
**Read `CLAUDE.md` first** — it is the canonical agent guide: commands, local stack, invariants, a doc map, project skills/agents, and a common-tasks table. Its Boost block (also inlined below) carries `.ai/guidelines/` — architecture incl. a "where things live in `app/`" map, code style, frontend, testing.

Then, before working in an area, read its domain doc:

- `.ai/domain/battlefield.md` — Phaser scene, sprites, snapshot/teardown, companions + subagent minion swarm
- `.ai/domain/token-tracking.md` — hook → EventController → damage pipeline, providers, per-event model, subagent tracking
- `.ai/domain/broadcasting.md` — PHP↔JS broadcast contract
- `.ai/domain/accounts.md` — org accounts (Claude + Codex), attribution, provisioning, quota probing, rebalance

Also check for a `CLAUDE.md`/`README.md` next to the code you touch (`resources/js/battlefield/`, `public/assets/battlefield/`, `extensions/*/`).

## Non-negotiables

1. TDD: failing test first, watch it fail, then implement (`.claude/skills/tdd/SKILL.md`).
...
8. Redis must be running (queue + subagent presence); tests fake it with `fakeRedis()`.
