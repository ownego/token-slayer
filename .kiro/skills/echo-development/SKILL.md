...
```bash
spin exec php php artisan install:broadcasting
```
...
```bash
spin exec php php artisan make:event OrderShipped
```
...
```bash
spin exec php php artisan make:channel OrderChannel
```

List all registered channels:

```bash
spin exec php php artisan channel:list
```
...
```bash
spin exec php php artisan queue:work    # Required for ShouldBroadcast events

spin exec php php artisan reverb:start  # Required for Reverb driver
...
- Channel classes via `spin exec php php artisan make:channel` — use for complex authorization logic that benefits from dependency injection or reusable logic across channels.
...
- Reverb is long-running: Code changes require `spin exec php php artisan reverb:restart`.
