...
- **Provisioned slots (`token-slayer setup`) only self-refresh while they are the active credential, and only the first account of a multi-account pull ever gets a refresh token at all.** Two related v1 limitations, both rooted in the slot store holding only the access token (`Account.token`), never the refresh token:
  1. **Switch away and back drops the refresh token.** `setup` writes the FULL grant (real refresh token, via `credstore.write_active_full`) for the account it activates, so Claude Code self-refreshes it directly. But `switch_to` always calls `credstore.write_active_token` (null refresh, ccm-style), so switching to another slot and back re-installs the provisioned one with `refreshToken: null` — it can no longer self-refresh until the user re-runs `setup`.
  2. **Multi-account `setup` only makes the FIRST account self-refreshing.** `pull_and_setup` calls `write_active_full` (real refresh) for `index == 0` only; accounts 2..N get a slot with just the access token, and their refresh token — served once from the now-consumed cache — is not persisted anywhere. So a secondary provisioned account works only until its ~8 h access token expires, then needs a re-`setup` (which re-provisions from the server). In practice the admin provisions one account per user, so the single-account flow (fully self-refreshing) is the norm; multi-account-in-one-pull is the degraded case.

  Accepted v1 limitations; not bugs to fix here. The proper fix (persist the full grant per slot so any provisioned account self-refreshes across switches) requires extending the slot schema and is deferred.
