...
- **Provisioned slots (`token-slayer setup`) lose their real refresh token on the next switch away and back.** `setup` writes the FULL grant (real refresh token, via `credstore.write_active_full`) so Claude Code self-refreshes it directly. But the slot's `Account.token` field only ever holds the access token, and `switch_to` always calls `credstore.write_active_token` (null refresh token, ccm-style). So switching to another slot and back to a provisioned one re-installs it with `refreshToken: null` — Claude Code can no longer self-refresh it until the user re-runs `setup`. Accepted v1 limitation; not a bug to fix here.
