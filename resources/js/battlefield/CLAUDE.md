...
| `boss/boss-type.js` | Pure sprite resolution: `bossTypeOf({number, name})` (a `fixedName` entry by name, else rotation), `bossTypeForNumber`, `BOSS_ROTATION` (entries without `fixedName`). Every sprite lookup goes through `Boss.bossTypeOf(bossState)` |
