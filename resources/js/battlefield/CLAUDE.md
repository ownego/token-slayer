...
| `minions.js` | `class Minions` — per-fighter subagent-count minion swarm (`demon-a`/`blood-monster-a`/`smw-goomba`/`smw-babybowser`/`smw-bowser`, each scaled by its own `charHeight`), independent world-space sprites repositioned each frame from `scene.js`'s `update()`: idle home-slot wander (`minion-layout.js`, biased into near-fighter gathering zones via `minion-grouping.js` when another fighter is close, radius reaching toward the zone's own nearest-neighbor midpoint) or trail-follow (`minion-trail.js`) depending on the fighter's own sampled speed; resyncs from its real position on the moving→idle transition so arriving doesn't snap the swarm; every rendered position step (trail sample or idle target alike) is capped at the fighter's own real speed (`MINION_WALK_SPEED_PX_PER_S` floor) so movement onset, a zone appearing/dissolving, or a neighbor leaving all read as a normal walk, never a "vút" dash. Each minion also carries a small avatar badge (the owning fighter's own avatar texture) above its head, plus a pulsing cyan ring (`handleAgentToolUsed`, `minion.assignedAgentId`, hook v7's `FighterAgentToolUsed`) on whichever ONE minion is currently assigned to a busy `agent_id`. Cross-fighter fights: one side attacks (weighted by who outnumbers whom in the area — `minion-fight.js`) with a random entry of its type's `attacks` (travelling to the target for `travel` ones), the victim plays its `reaction` at the attack's `hitFrame`, plus a staggered multi-burst from `MINION_CLASH_EFFECTS` (`config/companions.js`); see `.ai/domain/battlefield.md` Companions |
| `minion-fight.js` | Pure `countSidesNear`, `pickAttackerSide`, `travelLanding`, `pickRandom`, `fidgetAttacks`, `flipToFace`, `flipToFaceAngle` — who attacks in a minion fight (3 vs 7 → 30/70), where a leap/dash lands beside its target, which attacks may play as an idle fidget, which way a minion faces (strips are drawn facing right) |
...
| `tests/js/config.test.js` | BOSS_TYPES, FIGHTER_TYPES, MINION_TYPES config shape validation (minion strips on disk, one frame size per type, attacks' hitFrame/travel inside their strip, reaction lasts ≥ ~1s) |
...
| `tests/js/minion-fight.test.js` | countSidesNear, pickAttackerSide, travelLanding, pickRandom, fidgetAttacks, flipToFace, flipToFaceAngle — area counting, weighted attacker roll, landing point, fidget filter, facing |
