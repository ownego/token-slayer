...
<div style="border-radius:.6rem; padding:.85rem 1rem; {{ $cardStyle }}">
    ...
    <div style="margin-top:.7rem; display:flex; flex-direction:column; gap:.6rem;">
        @foreach (['5h' => $g['util_5h'], '7d' => $g['util_7d']] as $label => $pct)
            <div>
                <div style="display:flex; justify-content:space-between; align-items:baseline; font-size:.72rem; opacity:.7; margin-bottom:.25rem;">
                    <span style="text-transform:uppercase; letter-spacing:.04em;">{{ $label }}</span>
                    <span style="font-variant-numeric:tabular-nums; font-weight:600; opacity:.9;">{{ $pct === null ? '—' : $pct.'%' }}</span>
                </div>
    ...
    <div style="margin-top:.7rem; font-size:.7rem; opacity:.55;">
        @if ($g['reset_7d_at'])
            7d resets {{ $g['reset_7d_at']->diffForHumans() }}@if ($g['projected_7d'] !== null) · projected {{ $g['projected_7d'] }}%@endif
        @else
            Never probed
        @endif
    </div>
