...
- `BossSpawned` additionally carries whatever `BossCharacter::of($boss)?->scriptState()` returns (the character is recognized by `boss_name`) for the boss's client-side script (today `stones` and a nullable `next_stone_at` for `boss-thanos`); the keys are **absent**, not null, for a scriptless boss. The engine never reads them by name: the boss's script translates the payload through its `readState(payload)` hook (`boss/scripts/thanos.js`), so an older client, or a boss with no script, simply shows no sockets.
